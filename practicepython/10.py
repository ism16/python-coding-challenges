...
if __name__ == "__main__":
    a = [random.randint(0,100) for x in range(0, random.randint(1,100))] 
    b = [random.randint(0,100) for x in range(0, random.randint(1,100))]

    print(a, '\n', '-'*50, '\n', b, '\n\n')


    equals = list(set(sorted([x for x in a for y in b if x == y])))
    print(equals)
